...
HRESULT CBossLobber::Initialize(void)
{	
...
int	CBossLobber::Progress(void)
{
	...
	if(m_eState == MONSTER_EGG)
	{
		...
		if(m_dwTargetTime + m_iEggTime < GetTickCount())
		{
			m_dwTargetTime = GetTickCount();
...
void CBossLobber::Render(void)
{
	...
	if(m_eState == MONSTER_EGG)
	{
		...
		D3DXMatrixTranslation(&matTrans, m_tInfo.vPos.x - g_Scroll.x, m_tInfo.vPos.y - g_Scroll.y - 25.f + m_fJump, 0.f);
		...
		D3DXMatrixTranslation(&matTrans, m_tInfo.vPos.x - g_Scroll.x, m_tInfo.vPos.y - g_Scroll.y - 75.f + m_fJump, 0.f);
...
void CBossLobber::FrameMove(void)
{
	if(m_bCreate)
	{
		return;
...
void CBossLobber::AstarMove(void)
{
...
void CBossLobber::SetAngle(D3DXVECTOR3 vDir)
{
...
void CBossLobber::SyncHpBar(void)
{
...
void CBossLobber::HitAction(int Att)
{
